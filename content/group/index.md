...
#### Ziyang Hang - Master Thesis (November 2023)
  ...
  - Superconducting coils are investigated for active shielding against cosmic radiation by generating strong magnetic fields around spacecraft. Charged particles in space like protons and various ions can be deflected by the generated field. Both NASA and ESA (the European Space Agency) have funded projects utilizing magnetic fields to deflect incoming cosmic radiation. Several NASA projects have substantiated the efficacy of active shielding in mitigating cosmic radiation on spacecraft. A set of eight elliptical superconducting coils arranged in a Halbach array is considered in this thesis and the magnetic performance of a single coil is explored using a combination of analytical expressions and an Ansys Maxwell simulation. The mean dimensions of the elliptical coils are 4 m and 2 m respectively for the major and minor radii. To reduce the cosmic radiation to required levels using the Halbach array, the total current threading through the cross section of each winding pack needs to be 1 × 10⁷ A. The geometrical configuration of the superconducting winding pack can be specified by the number of turns, number of layers, the conductor size, and the size of the thermal and structural-support members. The influence of these parameters on magnetic field performance has been investigated. Two different conductor designs have been explored. The first utilizes a YBCO tape with a thickness and width of 0.1 mm and 12 mm respectively, while the second incor- porates a 8 mm diameter CORC cable comprised of 48 YBCO tapes and a copper core. The advantages and disadvantages of each option are detailed and discussed.

### Medical Physics Student
#### Ohyun Kwon - PhD Student (2023)
  - Work on [Geant4](https://geant4.web.cern.ch/) particle physics Monte Carlo simulation software to estimate
radiation exposure for astronauts in space. The software framework‬‭ computes‬‭ collisions‬‭ of‬‭ cosmic-ray particles‬‭ with‬‭ a‬‭ spacecraft‬‭ to‬‭ generate‬‭ secondary‬‭ particles‬‭ and‬‭ energy‬‭ deposits‬‭ of‬‭ these‬‭ "particle‬‭ cascades"‬‭ in‬‭ the‬‭ matter‬‭ they‬‭ traverse.‬‭ The‬‭ International‬
‭Commission‬‭ on‬‭ Radiological‬‭ Protection‬‭ ([ICRP](https://www.icrp.org/))‬‭ Adult‬‭ Male‬‭ Computational‬‭ Phantom‬‭ ([ICRP‬
‭ 110](https://www.icrp.org/publication.asp?id=icrp%20publication%20110))‬‭ is‬‭ used‬‭ to‬‭ estimate‬‭ the‬‭ radiation‬‭ exposure‬‭ on‬‭
the‬‭ human‬‭ crew,‬‭ representing‬‭ the‬‭ realistic‬ human‬‭ body‬‭ with‬‭ all‬‭ its‬‭ internal‬‭ organs.‬‭ The‬‭ energy‬‭ deposited‬‭ in‬‭ each‬‭ organ‬‭
is‬‭ recorded‬‭ and‬ converted‬‭ into‬‭ an‬‭ equivalent‬‭ dose‬‭ via‬‭ radiation‬‭ weighting‬‭ factors,‬‭ which‬‭ allows‬‭ the‬ fluence-to-dose conversion coefficients to be calculated. The exposure calculated with geant4 is compared to that from the NASA On-Line Tool for teh Assessment of Radiation in Space ([OLTARIS](https://oltaris.nasa.gov/)).
